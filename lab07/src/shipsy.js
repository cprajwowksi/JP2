...
class IceBreakerShip extends Ship{
    ...
    sail(){
        (this.direction === 'N' ? ((this.grid.grid[this.x-1][this.y].type === 'WATER' || this.grid.grid[this.x-1][this.y].type === 'ICE') ? this.x = this.x - 1 : super.turn())
        : this.direction === 'E' ? (this.grid.grid[this.x][this.y+1].type === 'WATER' || this.grid.grid[this.x][this.y+1].type === 'ICE'? this.y = this.y + 1 : super.turn())
        : this.direction ===' S' ? (this.grid.grid[this.x+1][this.y].type === 'WATER' || this.grid.grid[this.x+1][this.y].type === 'ICE'? this.x = this.x + 1 : super.turn())
        : ((this.grid.grid[this.x][this.y-1].type === 'WATER' || this.grid.grid[this.x][this.y-1].type === 'ICE') ? this.y = this.y - 1 : super.turn()));
    }
}
...
icebreaker.sail(); // Statek nacpotkał lądolód, przedarł się przez niego
